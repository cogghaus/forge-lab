...
## Issue 50: build images in CI, push to GHCR, accserver only pulls

Today `cd.yml` runs `docker compose up -d --build` for 4 images (hub, dash, mcp,
daemon) plus the waker on the accserver self-hosted runner. The builds crushed
...
- Registry hygiene: images are org-internal packages; first push must be
  followed by (manual, Adam or gh api) granting the accserver runner read
  access if the packages default to private. Document in the workflow header.

Non-goals: no runner watchdog, no multi-arch (accserver is linux/amd64 only).
...
Compatibility: hub and daemons deploy together on accserver (single compose
