...
export async function createTaskAction(
  ...
  const taskKind = (formData.get('taskKind') as 'coding' | 'review' | null) ?? undefined;
  const reviewConfig = (formData.get('reviewConfig') as string | null) ?? null;
  ...
  const res = await hubFetch<{ id?: string; error?: string }>(
    `/workspaces/${workspaceId}/tasks`,
    {
      ...
      body: {
        ...
        ...(reviewConfig !== null ? { reviewConfig } : {}),
      },
