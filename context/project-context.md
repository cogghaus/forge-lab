...
- **Current state:** Deployed and operational, not a Phase 1 vertical slice. 9 agent
  daemons run in production on accserver (192.168.66.220), dashboard live at
  `https://lab.local.cogg.haus`, hub is on migration `0017` (18 hand-written
...
- Live in production at `https://lab.local.cogg.haus`; boots locally with `pnpm dev` (port 3001). Not future work.
...
│   └── cd.yml             # deploy to accserver on merge to main
